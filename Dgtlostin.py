...
\033[1;31m╚═════╝ \033[1;33m ╚═════╝ \033[1;32m   ╚═╝   \033[1;36m╚══════╝\033[1;34m ╚═════╝ \033[1;35m╚══════╝\033[1;31m   ╚═╝   \033[1;33m╚═╝╚═╝  ╚═══╝
\033[0m
\033[1;36m[+]💻 Developer : Aryan Afridi\033[0m
\033[1;33m[+]🔴 YouTube   : https://www.youtube.com/@aryanafridi00\033[0m
\033[1;32m[+]🌐 GitHub    : https://github.com/shahid2005a\033[0m
"""
    print(banner)
...
def check_number():
    ...
    if success and "data" in data:
        ...
        sim_card = info.get("SIM Card", "N/A")
        complaints = info.get("Complaints", "N/A")
        reference_city = info.get("Reference City", "N/A")
        tower_locations = info.get("Tower Locations", "N/A")
        owner_personality = info.get("Owner Personality", "N/A")

        print(f"\n{BOLD}{BLUE}👑 Owner Information:{RESET}")
        print(f"   {YELLOW}🤵 Name           :{RESET} {owner_name}")
        print(f"   {YELLOW}📍 Address        :{RESET} {owner_address}")
        print(f"   {YELLOW}📱 Phone          :{RESET} {owner_phone}")
        print(f"   {YELLOW}🎭 Personality    :{RESET} {owner_personality}")

        print(f"\n{BOLD}{BLUE}📲 Device & Network:{RESET}")
        print(f"   {YELLOW}🔢 IMEI Number    :{RESET} {imei}")
        print(f"   {YELLOW}📡 MAC Address    :{RESET} {mac}")
        print(f"   {YELLOW}🌐 IP Address     :{RESET} {ip}")
        print(f"   {YELLOW}📶 SIM Card       :{RESET} {sim_card}")
        print(f"   {YELLOW}🛜 Connection     :{RESET} {connection}")

        print(f"\n{BOLD}{BLUE}📍 Location Details:{RESET}")
        print(f"   {YELLOW}📲 Mobile Locations :{RESET} {location}")
        print(f"   {YELLOW}🏠 Hometown         :{RESET} {hometown}")
        print(f"   {YELLOW}🏙️ Reference City   :{RESET} {reference_city}")
        print(f"   {YELLOW}🗽 State            :{RESET} {state}")
        print(f"   {YELLOW}🌎 Country          :{RESET} {country}")
        print(f"   {YELLOW}🗼 Tower Locations  :{RESET} {tower_locations}")

        print(f"\n{BOLD}{BLUE}🔍 Tracking & Reports:{RESET}")
        print(f"   {YELLOW}🆔 Tracker ID       :{RESET} {track_id}")
        print(f"   {YELLOW}🚨 Tracking History :{RESET} {tracking_history}")
        print(f"   {YELLOW}⚠️ Complaints       :{RESET} {complaints}")

        print(f"\n{BOLD}{BLUE}📑 Other Information:{RESET}")
        print(f"   {YELLOW}🗣️ Language         :{RESET} {language}")
